...
int PROCESS::start(int argc,char** argv)
{
    ...
    while((opt=getopt_long(argc,argv,"912Vfrt:p:c:?h",long_options,&options_index))!=EOF )
    {
        ...
        case 'p': 
            ...
            cout << "120proxyhost:proxyport = " << proxyhost << ":" << proxyport<<endl; 
            break;
        case ':':
        case 'h':
...
bool PROCESS::bench(void)
{
    ...
    cout << "****** url ******" << endl;
    cout << host<<endl;
    cout << "    speed = " << (allspeed+allfailed)/(benchtime/1.0) << "pages/min"<<endl;
